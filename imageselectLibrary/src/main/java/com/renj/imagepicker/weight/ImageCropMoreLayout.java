...
public class ImageCropMoreLayout extends LinearLayout implements View.OnClickListener {
    ...
    public void setImageData(List<ImageModel> srcImages) {
        ...
        if (ConfigUtils.isShowLogger())
            ConfigUtils.i("裁剪：(" + (currentIndex + 1) + " / " + srcImages.size() + ")");
                    ...
                    handler.post(new Runnable() {
                        ...
                        public void run() {
                            if (currentIndex < srcImages.size()) {
                                ...
                                tvCrop.setText("(" + (currentIndex + 1) + " / " + srcImages.size() + ")裁剪");
                                if (onCropImageChange != null) {
                                    onCropImageChange.onCropChange(tvCrop, tvCancel, imageModel, resultImages, imagePickerParams.isOvalCrop(), (currentIndex + 1), srcImages.size());
                                }
                            }
                        }
                    });
     ...
     */
    public void setClipViewParams(@NonNull ImagePickerParams imagePickerParams) {
